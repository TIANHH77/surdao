...
import pandas as pd
import polars as pl  # <--- ESTO ES LO QUE TE FALTA
import requests
import time
...
# ==========================================
# PESTAÑA 4: AUDITORÍA TERRITORIAL (EL MAPA INTELIGENTE)
# ==========================================
with tab4:
    ...
    @st.cache_data
    def cargar_geo():
        return pl.read_parquet(r"C:\PROYECTOS\streamlit\matriz_final_geolocalizada.parquet")
    ...
    # Usamos .copy() para poder modificar el dataframe sin que Pandas nos lance advertencias
    df_filtrado = df_pd[df_pd['Anio'] == anio_mapa].copy()
    
    # ---------------------------------------------------------
    # 🔥 LA MAGIA: ASIGNACIÓN DINÁMICA DE COLOR SEGÚN LA OCDE
    # ---------------------------------------------------------
    def asignar_color(ratio):
        if pd.isna(ratio):
            return [150, 150, 150, 100]  # Gris si no hay dato
        elif ratio <= 20:
            return [46, 204, 113, 160]   # Verde: Zona Tolerable / Óptima
        elif ratio <= 30:
            return [241, 196, 15, 180]   # Amarillo/Naranja: Zona de Alerta
        else:
            return [255, 40, 40, 200]    # Rojo Sangre: Colapso Estructural
            
    # Creamos una nueva columna 'color' evaluando cada fila
    ...
    st.pydeck_chart(pdk.Deck(
        ...
        layers=[
            pdk.Layer(
                ...
                get_color='color', # <--- AQUÍ LE DECIMOS QUE USE LA COLUMNA QUE CREAMOS
                get_radius=200,
                radius_min_pixels=1,
